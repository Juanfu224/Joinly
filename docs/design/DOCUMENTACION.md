...
- [6. Responsive Design](#6-responsive-design)
  - [6.1 Sistema de Breakpoints](#61-sistema-de-breakpoints)
  - [6.2 Estrategia Mobile-First](#62-estrategia-mobile-first)
  - [6.3 Container Queries](#63-container-queries)
  - [6.4 Adaptaciones por Viewport](#64-adaptaciones-por-viewport)
  - [6.5 Páginas Adaptadas](#65-páginas-adaptadas)
  - [6.6 Testing de Responsive](#66-testing-de-responsive)
...
El sistema de breakpoints sigue una estrategia Mobile-First con los siguientes valores:

| Breakpoint | Variable SCSS | Valor | Dispositivo |
|------------|---------------|-------|-------------|
| Mobile Small | `$bp-mobile-small` | 320px (20rem) | iPhone SE, dispositivos pequenos |
| Mobile | `$bp-movil` | 640px (40rem) | Mobile grande, landscape |
| Tablet | `$bp-tablet` | 768px (48rem) | iPad portrait |
| Desktop | `$bp-desktop` | 1024px (64rem) | Laptop, iPad landscape |
| Big Desktop | `$bp-big-desktop` | 1280px (80rem) | Desktop grande |

**Justificacion:**

- **320px**: Minimo viable para dispositivos pequenos (iPhone SE). Garantiza accesibilidad.
- **640px**: Punto donde el contenido puede beneficiarse de layouts de 2 columnas.
- **768px**: Estandar de la industria para tablets en modo portrait.
- **1024px**: Punto donde la navegacion desktop es preferible al menu hamburguesa.
- **1280px**: Desktop estandar donde se pueden mostrar mas elementos.

### 6.2 Estrategia Responsive: Mobile-First

Se utiliza la estrategia **Mobile-First** por las siguientes razones:

1. **Rendimiento**: Los estilos base son mas ligeros, los dispositivos moviles no descargan CSS innecesario.
2. **Priorizacion de contenido**: Obliga a disenar primero para el caso de uso mas restrictivo.
3. **Progressive Enhancement**: Se agregan funcionalidades segun aumenta el espacio disponible.
4. **Compatibilidad**: Los navegadores antiguos obtienen la version basica funcional.

**Ejemplo de implementacion:**

```scss
// Mixin responder-a() en _mixins.scss
@mixin responder-a($punto-ruptura) {
...
Se implementan Container Queries en componentes que se usan en diferentes contextos de layout, permitiendo que se adapten al contenedor padre en lugar del viewport.

#### Componentes con Container Queries implementadas:

| Componente | Container Name | Breakpoints |
|------------|----------------|-------------|
| `card` (variante feature) | `card` | 300px, 400px |
| `card` (variante list) | `card` | 600px |
| `group-card` | `group-card` | 280px, 360px |
| `subscription-card` | `subscription-card` | 280px, 320px |
| `member-list` | `member-list` | 400px, 600px, 800px |

**Ejemplo de implementacion (group-card):**
...
.c-group-card {
  // Estilos base (mobile pequeno)
  padding: var(--espaciado-2) var(--espaciado-3);
  min-block-size: 140px;

  // Container Query: >= 280px
  @container group-card (min-width: 17.5rem) {
    padding: var(--espaciado-3) var(--espaciado-4);
    min-block-size: 160px;
  }

  // Container Query: >= 360px
  @container group-card (min-width: 22.5rem) {
...
- Los componentes son independientes del viewport
- Se pueden reutilizar en sidebars estrechos o contenidos amplios
- Mejor encapsulacion de la logica responsive

### 6.4 Adaptaciones Principales

| Componente/Pagina | Mobile (320-639px) | Tablet (768-1023px) | Desktop (1024px+) |
|-------------------|-------------------|---------------------|-------------------|
| **Header** | Menu hamburguesa, logo | Menu hamburguesa, logo | Navegacion inline completa |
| **Footer** | 1 columna apilada | 2 columnas | 3 columnas con navegacion |
| **Home Hero** | Texto centrado, tamaño reducido | Texto mayor, padding aumentado | Tipografia completa (64px) |
| **Features Grid** | 1 columna | 2 columnas | 3 columnas |
| **Dashboard Acciones** | Apiladas verticalmente | Lado a lado | Lado a lado |
| **Dashboard Grid** | 1 columna | 2 columnas | 3 columnas |
| **Grupo Detalle Header** | Centrado, elementos apilados | Flex row con justify-between | Layout completo |
| **Suscripciones Grid** | 1 columna | 2 columnas | 3-4 columnas |
| **Miembros Grid** | 1 columna | 2 columnas | 3-4 columnas (via Container Query) |
| **Formularios** | Ancho completo, botones apilados | Ancho contenido, botones row | Max-width 650px centrado |
| **Cards** | Adaptacion via Container Query | Adaptacion via Container Query | Adaptacion via Container Query |

### 6.5 Paginas Implementadas

| Pagina | Ruta | Descripcion |
|--------|------|-------------|
| **Home** | `/` | Landing page con hero y features responsive |
| **Dashboard** | `/dashboard` | Grid de grupos con acciones adaptativas |
| **Grupo Detalle** | `/grupo/:id` | Detalle con miembros y suscripciones en grid |
| **Suscripcion Detalle** | `/grupo/:id/suscripcion/:id` | Stats cards y miembros responsive |
| **Login** | `/login` | Formulario centrado con fondo morado |
| **Register** | `/register` | Formulario centrado con fondo naranja |
| **Crear Grupo** | `/crear-grupo` | Formulario centrado con fondo naranja |
| **Unirse Grupo** | `/unirse-grupo` | Formulario centrado con fondo azul |
| **Crear Suscripcion** | `/grupo/:id/crear-suscripcion` | Formulario centrado con fondo azul |
| **Como Funciona** | `/como-funciona` | Pasos con iconos adaptivos |
| **FAQ** | `/faq` | Accordion con navegacion por fragments |
| **Terminos** | `/terminos` | Contenido legal con tipografia adaptiva |
| **Privacidad** | `/privacidad` | Contenido legal con tipografia adaptiva |

### 6.6 Testing Responsive

Viewports verificados en Chrome DevTools y Firefox Developer Tools:

| Viewport | Resolucion | Estado |
|----------|------------|--------|
| Mobile pequeno | 320px | Verificado |
| Mobile estandar | 375px | Verificado |
| Tablet | 768px | Verificado |
| Desktop pequeno | 1024px | Verificado |
| Desktop estandar | 1280px | Verificado |

**Herramientas utilizadas:**
- Chrome DevTools Device Mode
- Firefox Responsive Design Mode
- Extension "Responsive Viewer" para comparativas simultaneas
